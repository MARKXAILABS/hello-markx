...
| SCALE-05 | ~~Transcript-tier spend reaches `cost-ledger.jsonl`~~ — **RETIRED as a Phase-3 verification (round-3 finding #13).** Widening the ledger to transcript-tier engines was DESCOPED: `floor/boot.ts`'s `if (sample?.sessionId) hive.appendCostLedger(sample)` and `hive.ts`'s `private applyCostRow` stay byte-unchanged, and only the DISPLAY join changed (03-02 carries positive assertions pinning both). Nothing in Phase 3 makes this behaviour true, so requiring it here would report the contract satisfied for work no plan does. It is the **declared Accepted Residual owned by PARITY-02** (D-03: 7 of 11 engines are `costTracking:'none'`; plan 02-12 owns the `REQUIREMENTS.md:145`/`:452` restatement, and **Phase 3 must not edit those two lines**). What Phase 3 verifies instead is the row below. | — | none — retired, not deferred | n/a |
