...
nyquist_blocker: "five of the seven Manual-Only rows are still open — see § Validation Sign-Off"
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|--------|
| 04-01-T1 | 04-01 | 1 | GATE-04 | T-04-SBX-02 | The `--add-dir` verdict is measured with a negative control, its `codex exec`-only scope is stated in the file, and codex's own shell tool name plus the JSON type of its `command` argument are recorded if the transcript shows them **Wave-7 status:** the artifact exists and is honest (`grep -c '^VERDICT: '` → **1**), but the verdict itself is **INCONCLUSIVE** — codex auth died before any model turn, so enforcement was never observed and codex's shell tool name / command JSON type are still UNMEASURED (04-06 ceiling item (s)). | spike | `grep -c '^VERDICT: ' .planning/phases/04-overnight-on-a-repo-that-matters/04-SPIKE-codex-sandbox.md` | ⚠️ |
| 04-01-T2 | 04-01 | 1 | GATE-03, GATE-05 | T-04-CMD-16 | A real shim child reaches a real `HookServer` **on win32**, selectable by shim constant, adding zero cases and zero skips | harness | `node test/gate-harness.cjs && node --test test/suite-integrity.test.cjs && npm test` | ✅ |
| 04-02-T1 | 04-02 | 1 | RECORD-01, RECORD-02 | T-04-LOG | One migration at `MIGRATIONS[2]`; `user_version` is 3 after reopen | unit | `node --test test/record-persist.test.cjs` | ✅ |
| 04-02-T2 | 04-02 | 1 | RECORD-01, RECORD-02 | T-04-LOG | The five `PersistStore` methods, with cached prepared statements | unit | `node --test test/record-persist.test.cjs test/record-retention.test.cjs` | ✅ |
| 04-02-T3 | 04-02 | 1 | RECORD-01, RECORD-02 | T-04-LOG | Row survives close/reopen **with `target` non-null**; a >16 MiB day reads back from its FIRST row; retention deletes only past the bound | unit | `node --test test/record-persist.test.cjs test/record-retention.test.cjs` | ✅ |
| 04-03-T1 | 04-03 | 1 | VIGIL-03 | T-04-BLK-02 | The blocked-agent test is **RED** against `delivery.ts:740`, naming `setStatus` and `WAKE_NUDGE`, with a control case | unit, **MUST be RED first** | `node --test test/delivery-main.test.cjs \|\| echo "EXPECTED RED"` | ✅ |
| 04-03-T2 | 04-03 | 1 | VIGIL-03 | T-04-BLK-01 | A blocked agent is not idled by quiesce and not mailed by the nudge; `blocked` is a **required** `DeliveryDeps` member | unit | `node --test test/delivery-main.test.cjs` | ✅ |
| 04-03-T3 | 04-03 | 1 | VIGIL-03 | T-04-BLK-02, T-04-BLK-05, T-04-BLK-06 | `matchBlockHint` strips, caps at 120 and bounds its window; `boot.ts:1056-1117` supplies a **real** producer reading `outputTail` | integration | `npm run typecheck && npm test && npm run lint` | ✅ |
| 04-04-T1 | 04-04 | 1 | VIGIL-04, VIGIL-02 | T-04-AGE-03 | `updatedAt?` and `released?` declared; a legacy `createdAt`-only card round-trips unchanged | unit | `npm run typecheck` | ✅ |
| 04-04-T2 | 04-04 | 1 | VIGIL-04 | T-04-AGE-09, T-04-AGE-10 | The stamp is in `writeTasks` (`hive.ts:2112-2126`), diff-driven, so the five direct callers are covered and an unchanged card keeps its value | unit | `node --test test/hive-task-mutation.test.cjs` | ✅ |
| 04-04-T3 | 04-04 | 1 | VIGIL-04 | T-04-AGE-01 | `add`/`patch`/`claim`/`done` **each** stamp ISO `updatedAt` ≥ `createdAt`; `writeTasks` driven directly with an unchanged-sibling assertion | unit | `node --test test/hive-task-mutation.test.cjs && npm test` | ✅ |
| 04-05-T1 | 04-05 | 1 | GATE-02 | T-04-ENV-01, T-04-ENV-02 | Allowlist keeps `PATH`/`HOME` and drops `AWS_SECRET_ACCESS_KEY`/`GH_TOKEN`/`ANTHROPIC_API_KEY`; mixed-case `Path` survives (L-13); `HTTPS_PROXY`/`NODE_EXTRA_CA_CERTS` survive (corporate egress); nine-item ceiling | unit | `node --test test/pty-env-allowlist.test.cjs` | ✅ |
| 04-05-T2 | 04-05 | 1 | GATE-02 | T-04-ENV-07, T-04-ENV-08 | The spawn env carries `HOME`/`USERPROFILE`, `TMP` and a planted `HIVE_CANARY_KEEP` **through the filter**, and no planted `CANARY_DROP` | integration (stubbed `nodePty.spawn`) | `node --test test/pty-spawn-env.test.cjs test/pty-env-allowlist.test.cjs && npm test` | ✅ |
| 04-05-T3 | 04-05 | 1 | GATE-02 | T-04-ENV-03 | **Manual-Only.** One live non-Claude agent authenticates and completes a card; `env` shows the canaries both ways **Wave-7 status:** halves A and B green against a real OS-delivered environment and a live authenticated **claude** agent; **half C (the non-Claude clause) is BLOCKED and UNVERIFIED** — codex and gemini both fail auth identically WITH and WITHOUT the filter, so attribution is proven and the clause is simply unrunnable here. | checkpoint | `npm test && npm run typecheck && npm run lint` + operator | ⚠️ |
...
| **04-06-T2** | 04-06 | 2 | GATE-03 | T-04-CMD-17, T-04-CMD-20, T-04-SNAP-08 | **GATE-03's entire call site**: **both** arms key on a command string, not `tool_name === 'Bash'` -- so GATE-01's path protection, `restore` included, reaches all seven engines rather than one; the shape judge runs before `protectedPathDenial` (call-to-call); `restore` is `rootTailVerdict`'s fifth literal; ceiling (j)-(v) | unit + repo-fact | `npm run typecheck && npm test` | ✅ |
| 04-06-T3 | 04-06 | 2 | GATE-03 | T-04-CMD-01 | A deny reaches a real engine over the real socket, including `curl \| sh` which the path gate alone would allow — **on win32, zero skipped** | integration | `node --test test/gate03-roundtrip.test.cjs && node --test test/suite-integrity.test.cjs && npm test` | ✅ |
| 04-07-T1 | 04-07 | 2 | VIGIL-03 | T-04-BLK-11, T-04-BLK-12 | `BLOCK_HINTS` exists once; the renderer's local declaration is **deleted**; the six engines it was never observed on are named beside it | unit | `npm run typecheck && node --test test/repo-claims.test.cjs` | ✅ |
| 04-07-T2 | 04-07 | 2 | VIGIL-03 | T-04-BLK-08 | Detection works with **no renderer module loaded** (`require.cache` scan); positive control; A8 bound; recovery via the window | unit | `node --test test/block-detect.test.cjs && node --test test/suite-integrity.test.cjs && npm test` | ✅ |
| 04-08-T1 | 04-08 | 2 | VIGIL-02 | T-04-CARD-01, T-04-CARD-02, T-04-CARD-11 | Card released in the teardown tick, naming the dropping agent, **synchronously**, through `deps.hive` — **with `AgentTeardownDeps.hive`'s TYPE widened in the same file** to name `patchTask` and the `tasks()` reader the runtime object already supplies, asserted by symbol boundary, `boot.ts`/`deps.ts` diffs still empty | unit | `npm run typecheck` | ✅ |
| 04-08-T2 | 04-08 | 2 | VIGIL-02 | T-04-CARD-03, T-04-CARD-07 | Follow-up patch carries `worktreeHasUnintegratedWork`'s branch/detail; `finalizeAgentWorktree` widened to take `deps`, its one caller in the same file | unit | `node --test test/agent-lifecycle.test.cjs` | ✅ |
| 04-08-T3 | 04-08 | 2 | VIGIL-02 | T-04-CARD-03, T-04-CARD-05 | A git failure loses the branch and never the release; `released.branch === undefined` strictly; no-card writes nothing | unit | `node --test test/agent-lifecycle.test.cjs && npm test` | ✅ |
| 04-09-T1 | 04-09 | 2 | RECORD-05 | T-04-SNAP-01..04, T-04-SNAP-09 | Restore 1 of 3; other 2 byte-identical; operator `status --porcelain` + `rev-parse HEAD` + branches unchanged; gitignored dir 0 entries; nested repo survives; `realpathSync.native` keying | integration | `node --test test/restore-points.test.cjs` | ✅ |
| 04-09-T2 | 04-09 | 2 | RECORD-05 | T-04-SNAP-07 | Store excluded twice (ignore seed + `UNTRACK_PATHS`); the timer is in `SHUTDOWN_STEPS` **and pinned by name** | unit | `node --test test/boot-floor.test.cjs && npm run typecheck` | ✅ |
| 04-09-T3 | 04-09 | 2 | RECORD-02 | T-04-LOG-07, T-04-LOG-08 | A day past 16 MiB reads back **whole, from its first row**, driven through the real `appendLog`; a failing store costs the mirror, never the JSONL | unit | `node --test test/record-retention.test.cjs && npm test` | ✅ |
| 04-10-T1 | 04-10 | 3 | GATE-03 | T-04-CMD-07, T-04-CMD-24 | pi's `HIVE_AUTO_APPROVE` branch is gated on main's verdict; **OpenCode sends `tool_input`** so there is something to judge; both marked, worded to distinguish a fixed defect from an unverifiable | unit | `npm run typecheck && npm test` | ✅ |
| 04-10-T2 | 04-10 | 3 | GATE-03 | T-04-CMD-08, T-04-CMD-17 | Grok's **real** translator (run as a child process) maps a main-side deny to `{decision:'deny'}` with the reason; a **codex-shaped** payload is denied; allow control; derived seven-engine reach | unit | `node --test test/engine-parity.test.cjs` | ✅ |
| 04-10-T3 | 04-10 | 3 | GATE-03 | T-04-CMD-09, T-04-CMD-23 | `MARKER_LEDGER` matches a live grep across all four pins; the honest claim is pinned **in both directions**, with a recorded mutation check | repo-fact | `node --test test/repo-claims.test.cjs && npm test` | ✅ |
| 04-11-T1 | 04-11 | 3 | VIGIL-01 | T-04-ABS-01, T-04-ABS-03, T-04-ABS-07 | Silence past threshold alarms **once** (1, then 0 across 5 ticks, then 1); fires when the **god** is the dead one, addressed to the operator, never `to:'god'`; payload names transition-time `doing` cards | unit | `node --test test/absence-watchdog.test.cjs` | ✅ |
...
| 04-11-T3 | 04-11 | 3 | VIGIL-01 | — | Whole-suite blast radius, with `index.ts`/`config.ts`/`sw.js` untouched | full-suite | `npm test && npm run typecheck && npm run lint` | ✅ |
...
| 04-12-T3 | 04-12 | 3 | VIGIL-04 | T-04-AGE-05 | Age renders on unanswered asks (4 min); the four-channel distinction asserted as ONE test for both surfaces | unit | `node --test test/renderer-components.test.cjs && npm test` | ✅ |
| 04-13-T1 | 04-13 | 4 | GATE-04 | T-04-SBX-03 | The codex preset yields `-s workspace-write --add-dir <agentDir>` on, and the bypass flag byte-identical off **Wave-7 status:** the assembler is correct and unit-green, but **no production caller passes `agentDir`** — every `buildSpawnCommand(config, model, provider)` site is three-argument — so with the opt-in on a real spawn yields `-s workspace-write` WITHOUT the agent folder added. Built, not wired. | unit | `npm run typecheck` | ⚠️ |
...
| 04-13-T3 | 04-13 | 4 | GATE-04 | T-04-SBX-05 | Exactly one engine row; the "other N" count derived from the preset table; the marker added iff the spike said so, with the ledger reconciled | unit + repo-fact | `node --test test/repo-claims.test.cjs && npm test` | ✅ |
| 04-13-T4 | 04-13 | 4 | GATE-04, **GATE-03**, **VIGIL-03** | T-04-SBX-06, T-04-SBX-10, T-04-CMD-25, T-04-BLK-13 | **Manual-Only.** A live **interactive** codex agent: the real spawned argv, a completed card, mail, `memory.md`, zero write-refusals with a positive control, the opt-in-off fallback — **plus a live GATE-03 refusal and a live VIGIL-03 block observation** **Wave-7 status: DID NOT RUN.** This machine's codex refresh token is revoked (`401 refresh_token_reused`, re-tested twice); no live agent ever started, so `LIVE GATE-03 REFUSAL: no` and `LIVE VIGIL-03 BLOCKED: no` record ABSENCE of evidence, not evidence of absence. | checkpoint | `node --test test/agent-provider.test.cjs test/repo-claims.test.cjs && npm test` + operator | ⚠️ |
| 04-14-T1 | 04-14 | 4 | GATE-03 | T-04-CMD-14 | The `destructive` variant reads 7.12:1 in dark mode (was 1.85:1) and is byte-identical in light; own atomic commit; pinned by a test | unit | `node --test test/renderer-components.test.cjs && npm run typecheck` | ✅ |
| 04-14-T2 | 04-14 | 4 | GATE-03 | T-04-CMD-13 | `BlockReason.command` is populated; the summary names the agent; main's reason renders verbatim; the invented fallback is deleted | unit | `node --test test/renderer-components.test.cjs` | ✅ |
| 04-14-T3 | 04-14 | 4 | VIGIL-03 | T-04-BLK-09, T-04-BLK-14 | `blocked` beats `armed` on the roster badge — four cases; `:796` **unchanged** and `repo-claims` diff empty (the aria rider is 04-18's) | unit + repo-fact | `node --test test/renderer-components.test.cjs test/repo-claims.test.cjs && npm test` | ✅ |
| 04-15-T1 | 04-15 | 4 | GATE-05 | T-04-ASK-01, T-04-ASK-04 | Unguessable single-use ids satisfying `PHONE_TASK_ID_RE`; expiry unanswerable; `sweep` settles once; **`ASK_TTL_MS ≤ PRETOOLUSE_HOOK_TIMEOUT_SEC × 1000`, both read from source** | unit | `node --test test/control.test.cjs` | ✅ |
...
| 04-16-T3 | 04-16 | 5 | GATE-05 | T-04-CMD-22 | All four ledger pins reconciled from a live grep in the same commit as the two markers | repo-fact | `node --test test/repo-claims.test.cjs && npm test` | ✅ |
...
| 04-17-T3 | 04-17 | 5 | GATE-05, VIGIL-01 | T-04-ASK-18, T-04-ASK-19 | `sw.js` changes by exactly one line; the push body carries no command, path or `?`, with a positive control; the title is not generic | unit | `node --test test/webhook-endpoints.test.cjs && npm test` | ✅ |
...
| **04-18-T4** | 04-18 | 6 | GATE-05, VIGIL-01 | T-04-ASK-25, T-04-ASK-27, T-04-ASK-38 | **Manual-Only.** A real ask answered at three widths in both themes; the four countdown states live; a wrapped command; **A9 focus retention demonstrated by tabbing after a click**; an expiry auto-denying; the chip's three degradation steps **Wave-7 status: NOT PERFORMED.** The checkpoint auto-approved under `auto_advance: true`; the seven demonstrations were never run. Items 1-5 need a live authenticated agent CLI (unavailable) and A9.2 focus retention has no automated proof in this repo — D-27 excludes jsdom/RTL/Playwright. | checkpoint | `node --test test/renderer-components.test.cjs && npm test` + operator | ⚠️ |
| 04-19-T1 | 04-19 | 7 | all eleven | T-04-DOC-01..03 | SECURITY.md names all four fail-opens with dispositions and owners and carries the honest claim verbatim; TELEMETRY.md states the ring is unchanged and `synchronous = NORMAL` | doc + full-suite | `npm test && npm run typecheck && npm run lint && npm run build` | ✅ |
...
| 04-19-T3 | 04-19 | 7 | all eleven | T-04-DOC-03 | **Manual-Only.** The physical-device attempt with its honest outcome; the kill-and-restart crash test; the eleven checkbox rulings **Wave-7 status: partially performed.** The kill-and-restart crash test RAN and PASSED against a real SIGKILL with a 160 KB uncheckpointed WAL; the **physical-Android half did NOT run — no device is reachable from this session** (`adb` absent, no Android SDK), recorded as a first-class outcome exactly as Phase 2 did for DAEMON-02. | checkpoint | `npm test && npm run typecheck && npm run lint && npm run build` + operator | ⚠️ |

*Status: ⬜ pending · ✅ green · ❌ red · ⚠️ green-with-a-recorded-caveat, flaky, or not fully exercised*

**Filled at the closing wave (plan 04-19, 2026-08-26) from runs in that session — 53 ✅ / 0 ❌ / 6 ⚠️.**
Every ✅ means every `Automated Command` on that row was executed here and exited 0; the 24 distinct
test files those commands name were each run individually and all 24 came back **0 fail, 0 skipped**.
Every ⚠️ carries its reason in its own `Secure Behavior` cell, and the six are the phase's honest
residue: one INCONCLUSIVE spike, one live-agent clause blocked by an environment fault with proven
attribution, one gate built but not wired, and three operator checkpoints whose manual halves did not
run. **The legend's third glyph is deliberately wider than "flaky":** nothing in this phase was flaky
— zero rows are red and no test failed once — and forcing a caveated row to ✅ or ❌ would have lost
exactly the information a closing map exists to carry.
...
**Ruled at the closing wave (plan 04-19, 2026-08-26). Every box below was decided against a command
run in that session; the numbers beside them are that session's output, not carried forward.**

- [x] All 59 tasks have an `<automated>` verify or a named Wave 0 dependency, and a row in the map above.
      Re-counted from the PLANS rather than from this file's own sign-off line:
      `grep -cE '^\s*<task type=' 04-*-PLAN.md` sums to **59** across the 20 plans, and
      `grep -cE '^\| (\*\*)?04-[0-9]+-T[0-9]+'` on this file returns **59**. Exact bijection.
- [x] All **59** map rows carry an outcome glyph in `Status` — one of ✅ / ❌ / ⚠️ — and none is still the
      draft word. Measured at close, row-scoped: **59** rows, **59** carrying an outcome glyph, **0**
      still reading `pending`, **0** with a placeholder coordinate. The split is **53 ✅ / 0 ❌ / 6 ⚠️**,
      and each ⚠️ carries its reason in the row itself. `⬜` occurs **only in the legend line**, never in
      a cell, so "not ⬜" is not a check; both counts above are anchored on `^| 04-NN-TN`, because every
      prose mention of a token in this file would otherwise satisfy a whole-file gate — the
      self-invalidating-prose trap this section's own rules name.
- [x] Sampling continuity: no 3 consecutive tasks without automated verify. Every one of the 59 rows
      carries an `Automated Command`, including the four checkpoint rows, so the longest run without one
      is **0**.
- [x] Wave 0 covers all ❌ references above — vacuous and stated as such: there are **zero ❌ rows**.
- [x] No watch-mode flags in any command. Measured: `grep -cE '\-\-watch'` on this file returns **0**.
- [x] Feedback latency re-measured at close and inside the 90 s phase budget. **Three quiet runs in the
      closing session: 23 729 / 24 087 / 23 768 ms — worst 24.1 s.** Delta from the **42.4 s worst-case**
      baseline: **−18.3 s** (faster), against a suite that grew from 805 to **1 078** cases. Headroom
      under the 90 s budget: **~66 s**. The two heaviest files were measured in case they ever threaten
      it: `engine-parity` 11.9 s and `gate05-bounded-wait` 10.5 s. Both integration sub-budgets hold —
      `gate05-bounded-wait` **10.5 s** against 15 s, `gate03-roundtrip` **1.4 s** against 10 s.
- [x] **Skipped count is unchanged at 7** — measured **7**, delta from the 7-skip baseline is **0**, and
      it is stated in the SUMMARY.
- [x] `node --test test/suite-integrity.test.cjs` green (**4 / 4 / 0 fail**), with `DECLARED_SKIPS` and
      `FROZEN` untouched — `git diff --stat <phase-base>..HEAD -- test/suite-integrity.test.cjs` is
      **empty** over the whole phase, and `FROZEN = { win32: 6, other: 1 }` still reads as written.
- [x] `wave_0_complete: true` set. Both of plan 04-01's deliverables exist and were exercised at close:
      `04-SPIKE-codex-sandbox.md` carries exactly **one** `VERDICT:` line, and `node test/gate-harness.cjs`
      self-checks **green on win32** (`HOOK_SHIM exit=0 115 ms`, `GROK_HOOK_SHIM exit=0 105 ms`).
- [x] `status` moved off `draft`, to `approved` — this file family's own convention for a validated,
      executed phase (`02-VALIDATION.md` uses exactly that value, and uses it beside a `false` nyquist
      flag, so the two are separate claims and are recorded separately here too).
- [ ] **`nyquist_compliant: true` — NOT SET. This criterion FAILS, and it is stated plainly rather than
      explained away.** There is no explained-exemption (M7), and none is being taken: the flag stays
      `false` because `false` is the true value, not because a paragraph excuses it.

      **The reason, measured rather than argued.** This workflow's own compliance rule — quoted verbatim
      in `02-VALIDATION.md` § Frontmatter corrections — reads: *"A phase with zero automated coverage,
      **or with Manual-Only items still open**, keeps `nyquist_compliant: false`."* **Five of this
      phase's seven Manual-Only rows are still open**, and none is closeable on this machine:

      | Open Manual-Only row | Requirement | Owning task | Blocker |
      |---|---|---|---|
      | Live codex agent finishes a task under the sandbox | GATE-04 | 04-13-T4 | `401 refresh_token_reused` — the stored ChatGPT refresh token is revoked; needs an operator browser sign-in |
      | A live agent has a command REFUSED by GATE-03 | GATE-03 | 04-13-T4 | same — no live agent ever started |
      | A live non-Claude agent on a prompt is (or is not) marked `blocked` | VIGIL-03 | 04-13-T4 | same |
      | GATE-05 approval answered on a **physical Android phone** | GATE-05 | 04-19-T3 | no device reachable from this session (`adb` absent, no Android SDK); also needs a fresh Quick-Tunnel pairing per open |
      | A9 focus retention, the click-driven action-row swap, the ticking countdown | GATE-05 | 04-18-T4 | `renderToStaticMarkup` fires no events and D-27 excludes jsdom/RTL/Playwright — no automated proof exists in this repo |

      Two of the seven are closed or explained: 04-05-T3's non-Claude clause is **BLOCKED with proven
      attribution** (both installed non-Claude engines fail auth identically with and without the
      filter), and the grok/pi/OpenCode row was **never runnable** and ships `LIVE-UNVERIFIED` markers
      with the ledger reconciled, which is the outcome its own instruction prescribes.

      **No plan-checker Dimension 8 confirmation exists for this phase.** Searched at close: the three
      red-team artifacts (`04-REDTEAM.md`, `-R2`, `-R3`) contain **zero** occurrences of "dimension", and
      nothing else in `.planning/` confirms it for phase 4. Plan 04-19's own conditional is explicit and
      is being honoured: *"if the checker has not confirmed, the flag stays `false`, task 3 blocks, and
      the phase does not close. That is an outcome, not a failure — but it is not a close."*

      **Precedent, so this is not read as a novel judgement.** Phase 2's post-execution audit moved this
      exact flag `true → FALSE` for this exact reason, and called it *"a correction in the honest
      direction"* — three of its five blockers (a physical Android phone, un-installed engine CLIs, a
      live authenticated non-Claude agent) are the same three blocking phase 4. Flipping it here would
      reverse that correction one phase later with strictly less evidence.

      **What would settle it, and it is cheap:** `codex login` (closes three rows at once), one physical
      Android device with a fresh QR pairing (closes the fourth), and either a browser-driver dependency
      or one recorded operator session (closes the fifth).
