...
export function realtimeReadTools(): ReturnType<typeof tool>[] {
  return [
    ...
    tool({
      name: 'get_app_info',
      description:
        'About the Hello MarkX app itself: the running version and the latest release notes (changelog). Use for "what version is this" or "what is new in this release".',
      parameters: { type: 'object', properties: {}, required: [], additionalProperties: false },
      execute: () =>
        spoken(async () => {
          const info = await window.cth.appInfo();
          const notes = despan(info.changelog || '');
          return `This is Hello MarkX version ${info.version}. ${notes ? `Latest release notes: ${clip(notes, 1600)}` : 'No release notes are bundled with this build.'}`;
        }, 'app info')
    })
  ];
}
